...
def test_filter_group_keeps_only_matching_date():
    ...
    result = formatting.filter_group_lessons_by_date(timetable, "2026.05.18")
...
def test_filter_group_empty_when_no_match():
    ...
    assert formatting.filter_group_lessons_by_date(timetable, "2026.05.18") == []


def test_filter_group_handles_non_list_input():
    assert formatting.filter_group_lessons_by_date("ошибка парсера", "2026.05.18") == []
    assert formatting.filter_group_lessons_by_date(None, "2026.05.18") == []
...
def test_filter_personal_keeps_only_matching_date():
    ...
    result = formatting.filter_personal_lessons_by_date(timetable, "2026-05-18")
...
def test_filter_personal_empty_when_no_match_or_empty():
    ...
    assert formatting.filter_personal_lessons_by_date(timetable, "2026-05-18") == []
    assert formatting.filter_personal_lessons_by_date([], "2026-05-18") == []
    assert formatting.filter_personal_lessons_by_date(None, "2026-05-18") == []
...
def test_week_offset_today_is_zero():
    ...
    assert formatting._week_offset_for_date(today, today) == 0


def test_week_offset_tomorrow_same_week():
    today = date(2026, 5, 20)  # среда
    assert formatting._week_offset_for_date(date(2026, 5, 21), today) == 0
...
def test_week_offset_tomorrow_crosses_into_next_week():
    ...
    assert formatting._week_offset_for_date(tomorrow, today) == 1


def test_week_offset_previous_week_is_negative():
    today = date(2026, 5, 20)  # среда
    assert formatting._week_offset_for_date(date(2026, 5, 11), today) == -1
