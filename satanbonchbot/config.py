...
# заголовков для всех клиентов ЛК/расписания (public_timetable, lk_client): раньше
...
# Период фонового рефреша расписания (часы, A.3). Каждый тик перечитывает
# расписание всех групп — дифф уведомлений об изменениях (C.1) считается
# регулярно, даже если расписание группы никто не открывал.
TIMETABLE_REFRESH_HOURS = max(0.1, float(os.getenv("TIMETABLE_REFRESH_HOURS", "6")))

# Период переопределения учебной группы пользователей (часы, A.3). Заметно
# реже рефреша: detect_user_group бьёт сетевым запросом в ЛК на каждого
# пользователя, а группа меняется ~раз в семестр. Округляется до целого
