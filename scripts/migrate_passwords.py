...
УЖЕ ВЫПОЛНЕНО в проде (фаза v1). Скрипт хранится здесь как референс и на
случай восстановления из старого бэкапа БД — повторный запуск идемпотентен.

Запуск из каталога проекта (НЕ из scripts/, путь к users.db относительный):
